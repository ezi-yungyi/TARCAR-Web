// components/forms/CreateAreaForm.tsx
...
interface CreateAreaFormProps {
  areas: Area[];
  onSubmit: (areaData: { name: string; campus_location: string; gps_locations: GPSLocation[]; nearBy: { id: number; name: string }[] }) => void;
}

const CreateAreaForm: React.FC<CreateAreaFormProps> = ({ areas, onSubmit }) => {
  // SECTION - Form Variables
  const [name, setName] = useState<string>('');
  const [campusLocation, setCampusLocation] = useState<string>('');
  const [nearBy, setNearBy] = useState<{ id: number; name: string }[]>([]);
  const [newNearBy, setNewNearBy] = useState<{ id: number; name: string } | null>(null);
  const [gpsLocations, setGpsLocations] = useState<{ latitude: number; longitude: number }[]>([]);
